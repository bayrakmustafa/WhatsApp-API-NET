	...
	const unsigned char* random)
{
	...
	if ((sigbuf = static_cast<unsigned char *>(malloc(msg_len + 128))) == 0) {
	...
	const unsigned char* msg, const unsigned long msg_len)
{
	...
	if ((verifybuf = static_cast<unsigned char *>(malloc(msg_len + 64))) == 0) {
		result = -1;
		goto err;
	}

	if ((verifybuf2 = static_cast<unsigned char *>(malloc(msg_len + 64))) == 0) {
		result = -1;
		goto err;
	}
